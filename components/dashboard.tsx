...
export default function Dashboard() {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      setRefreshing(true)
      const response = await fetch(apiUrl("products"), { signal: controller.signal })
      if (!response.ok) {
      ...
      setProducts(data)
      localStorage.setItem("dashboard_products_cache", JSON.stringify(data))
      ...
      // Keep cached/previous data visible for better perceived performance.
      if (err instanceof Error && err.name === "AbortError") {
        setError("Server is taking too long. Showing last available data.")
      } else {
  ...
  useEffect(() => {
    // Hydrate from cache first so dashboard paints immediately.
    try {
      const cached = localStorage.getItem("dashboard_products_cache")
      if (cached) {
        const cachedProducts: Product[] = JSON.parse(cached)
        setProducts(cachedProducts)
        const transformedData: DashboardItem[] = cachedProducts.map(product => ({
          name: product.name,
          count: product.stock,
          min: 5,
          price: product.price,
